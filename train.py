...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Train the LoRAniDiff model on specified dataset.")
    parser.add_argument("--dataset", type=str, choices=["pixiv", "textcaps"], required=True, help="Dataset to train on, either 'pixiv' or 'textcaps'.")
    parser.add_argument("--lr", type=float, default=1e-4, help="Learning rate.")
    parser.add_argument("--batch_size", type=int, default=2, help="Batch size.")
    parser.add_argument("--epochs", type=int, default=10, help="Number of epochs.")
    parser.add_argument("--alpha", type=float, default=0.5, help="Weight between reconstruction and CLIP-guided losses.")
...
def evaluate_model(model, data_loader, device):
    ...
    with torch.no_grad():
        for images, captions in data_loader:
            ...
            val_loss = model(images, captions)  # Assuming the model's forward method returns a loss
