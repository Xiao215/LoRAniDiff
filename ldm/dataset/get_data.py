...
def main():
    ...
    choice = input("Which dataset do you want to download? (pixiv [p]/textcaps [t]): ").strip().lower()
    ...
    if choice == 'pixiv' or choice == 'p':
        ...
        if os.path.exists(dataset_dir):
            print(f"The folder {dataset_dir} already exists. Please remove it before downloading the dataset.")
            return
        resize = input("Do you want to resize the images? If no, default is 360x360. (yes [y]/NO [n]): ").strip().lower()
        pixiv_script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'pixiv', 'pixiv_dataset.py')

        if resize == 'yes' or resize == 'y':
            size = input("Enter the size to which you want the images resized (e.g., 512): ").strip()
        ...
        if os.path.exists(dataset_dir):
            print(f"The folder {dataset_dir} already exists. Please remove it before downloading the dataset.")
            return
        textcaps_script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'textcaps', 'textcaps.sh')
